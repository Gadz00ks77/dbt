...
sum(case when tr.is_addition = 'Y' then transaction_orig_gross_amt else 0 end) as cumu_transaction_orig_amt_add, ---this is only here because I think the logic for additional calcs is poorly defined
sum(case when tr.is_addition = 'Y' then transaction_sett_gross_amt else 0 end) as cumu_transaction_sett_amt_add,
