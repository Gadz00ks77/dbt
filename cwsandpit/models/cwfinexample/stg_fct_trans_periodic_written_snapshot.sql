...
-- this summarises the written transactions into a snapshot day-by-day accumulating view, including the transaction event. 

with 

tr_in_range as (

select d.date::date as snapshot_date, * from {{ref('stg_fct_written_transactions')}} tr
...
select 
  ...
  *,
  --last_value(tr.inceptiondate::date) over (partition by tr.snapshot_date,tr.policyid order by tr.transaction_date) as last_inception_date, 
  --last_value(tr.expirydate::date) over (partition by tr.snapshot_date,tr.policyid order by tr.transaction_date) as last_expiry_date, -- INCEPTION / EXPIRY ETC TECHNICALLY NOT CORRECT
  --last_value(tr.yoa::int) over (partition by tr.snapshot_date,tr.policyid order by tr.transaction_date) as last_yoa,
  last_value(tr.assured) over (partition by tr.snapshot_date,tr.policyid order by tr.transaction_date) as last_assured, -- give the last appropriate value in the snapshot range (e.g. the last line record up to that snapshot date)
