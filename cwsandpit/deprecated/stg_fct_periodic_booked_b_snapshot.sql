...
select d.date::date as snapshot_date, * from {{ref('stg_fct_booked_b_transactions')}} tr
...
sum(case when tr.is_addition = 'Y' then transaction_orig_amt else 0 end) as cumu_transaction_orig_amt_add, ---this is only here because I think the logic for additional calcs is poorly defined
sum(case when tr.is_addition = 'Y' then transaction_ledger_amt else 0 end) as cumu_transaction_ledger_amt_add,
sum(transaction_orig_amt) as cumu_transaction_orig_amt,
sum(transaction_ledger_amt) as cumu_transaction_ledger_amt
...
select
'booked bureau' as measure_source, 
cu.source_system,
...
cumu_transaction_orig_amt_add,
cumu_transaction_ledger_amt_add,
cumu_transaction_orig_amt,
cumu_transaction_ledger_amt
